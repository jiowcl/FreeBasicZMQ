...
Sub TestZmqThreadRepMonitorProc(Byval vData As Any Ptr)
    ...
    For i = 1 To 10
        ...
        n = ZmqRecv(hLibrary, Socket, eventBuf, 6, ZMQ_DONTWAIT)
        If n < 6 Then
            Deallocate(eventBuf)
            Continue For
        End If
        ...
        ZmqRecv(hLibrary, Socket, @address, SizeOf(address), ZMQ_DONTWAIT)
...
If hLibrary > 0 Then
    ...
    ZmqSocketMonitor(hLibrary, SocketRep, lpszMonitorAddr, ZMQ_EVENT_ACCEPTED Or ZMQ_EVENT_CLOSED)
    ZmqBind(hLibrary, SocketRep, lpszServerAddr)

    SocketMonitor = ZmqSocket(hLibrary, ContextRep, ZMQ_PAIR)
    ZmqSetsockoptInt(hLibrary, SocketMonitor, ZMQ_LINGER, 0)
    ZmqConnect(hLibrary, SocketMonitor, lpszMonitorAddr)
    threadMonitor = ZmqThreadstart(hLibrary, @TestZmqThreadRepMonitorProc, SocketMonitor)

    ContextReq = ZmqCtxNew(hLibrary)
    SocketReq = ZmqSocket(hLibrary, ContextReq, ZMQ_REQ)
    ZmqSetsockoptInt(hLibrary, SocketReq, ZMQ_LINGER, 0)
    ZmqConnect(hLibrary, SocketReq, lpszServerClientAddr)

    threadRep = ZmqThreadstart(hLibrary, @TestZmqThreadRepProc, SocketRep)
    threadReq = ZmqThreadstart(hLibrary, @TestZmqThreadReqProc, SocketReq)
